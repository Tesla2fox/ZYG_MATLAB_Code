function[X CompleteTime endtime Th]=MPDAdecodeminmax(R,T)   %%时间复杂度为O(n^2)
tic
Com=[];         %%存储每一次的完成时间
XX=MPDAcodeanotherExpminmax(R,T);  %%时间复杂度为O(n^2)
% XX=MPDAcodeanotherExpThreeElements(R,T);
qq=11;
for r=1:qq
    TT=zeros(30000,8);   %TT=[rob task t到达 此刻任务点的变化率 此刻任务点状态值 预计任务完成时间 辅助 辅助]
