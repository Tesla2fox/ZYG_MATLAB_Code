function[X CompleteTime endtime Th]=MPDAdecodeDynamicSumMinmax(R,T)
tic
Com=[];         %%存储每一次的完成时间
% XX=MPDAcodeExpminmaxDynamic(R,T);
XX=MPDAcodeExpDynamicSumMinmax(R,T);    %%时间复杂度O(n^3)
% XX=MPDAcodeExpDynamicSumMinmaxS(R,T);    %%T
% XX=MPDAcodeExpDynamicSumMinmin(R,T);   %%T
% XX=MPDAcodeExpDynamicNsumMinmax(R,T);   %%最小最大，最不重要的机器人
qq=11;
for r=1:qq
    TT=zeros(15000,8);   %TT=[rob task t到达 此刻任务点的变化率 此刻任务点状态值 预计任务完成时间 辅助 辅助]
